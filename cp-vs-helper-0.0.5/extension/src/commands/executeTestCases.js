...
async function executeTestCases(jsonData) {
  ...
  if (!state.workspacePath || state.workspacePath.length === 0) {
    state.outputChannel.appendLine("The workspace path is not defined, execute the commands in sequence");
    return;
  }
  ...
  if (fs.existsSync(extraTestCasesFile)) {
    try {
      ...
      state.outputChannel.appendLine(`Added ${extraTests.length} extra test case(s) from testcases.txt.`);
    } catch (err) {
      state.outputChannel.appendLine("Error reading extra test cases: " + err.message);
    }
  }
  
  const compileCommand = `g++ -o "${executablePath}" "${solutionPath}"`;
  exec(compileCommand, { cwd: state.workspacePath }, (compileErr, _, compileStderr) => {
    if (compileErr) {
    ...
    tests.forEach((test, index) => {
      const terminal = spawn(`${executablePath}`, { cwd: state.workspacePath, shell: true });
      ...
      terminal.stderr.on("data", (data) => {
        state.outputChannel.appendLine(`Error executing test case ${index + 1}:\n` + data.toString());
      });
      ...
      terminal.on("close", () => {
        ...
        state.outputChannel.appendLine(`Test Case ${index + 1}: ${testStatus}`);
...
async function fetchDataToExecute() {
  try {
    ...
    const response = await axios.get("https://127.0.0.1:10042/bodydata");
